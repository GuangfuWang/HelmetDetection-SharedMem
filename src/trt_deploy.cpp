...
TrtDeploy::~TrtDeploy()
{
	...
	}
	{
		std::lock_guard<std::mutex> lock(m_mtx);
		m_thread_num--;
		if(enable_trt_log)std::cout << "Current remaining runtime ref count: " << m_thread_num << std::endl;
		if (m_thread_num == 0) {

			if(enable_trt_log)std::cout << "Free the runtime done..." << std::endl;
		}
	}
	if(enable_trt_log)std::cout << "Thread: " << std::this_thread::get_id() <<
...
void TrtDeploy::Init(const std::string &model_file)
{
	...
	ai_model.close();
	if(enable_trt_log)std::cout << "Model size: " << mSize/(1024*1024) <<"MB"<< std::endl;
...
//		std::lock_guard<std::mutex> lock(m_mtx);
		...
		if(enable_trt_log)printf("Logger: %p; Runtime: %p; Engine: %p; Context: %p\n",
