...
//    cv::Mat::setDefaultAllocator(cv::cuda::HostMem::getAllocator (cv::cuda::HostMem::AllocType::PAGE_LOCKED));
	...
	while (cap.isOpened()) {
		...
		int ss = img.total()*img.elemSize();
		std::cout<<"Img bytes: "<<ss<<std::endl;
		auto dur = std::chrono::high_resolution_clock::now() - curr_time;
		curr_time = std::chrono::high_resolution_clock::now();
		auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(dur).count();
		if (count % print_cnt == 0 && init){
	...
	if (count)
		std::cout << "Mean Time Consumption Info: Thread: " << std::this_thread::get_id() << " Cpu: " << sched_getcpu()
				  << " taken: "
				  << std::fixed << std::setprecision(2) << sum / count << "ms" << std::endl;
